...
### How to avoid sniping while self-minting?
*Sniping* is the name given by the community to the minting of kBTC / iBTC immediately following new locked collateral by a Vault. This prevents Vaults from maintaining a collateralization threshold that is higher than the system-defined [secure threshold](#secure-collateral). Vault operators have control over whether to accept new issue requests or not, so sniping is prevented when issue requests are disabled.

!> In the following example, the Vault is assumed to use `KSM` as collateral. If your Vault is using a different collateral currency, replace the value accordingly.

Go to polkadot.js.org/apps and deactivate new issue requests by submitting the following extrinsic: `Developer` -> `Extrinsics` -> `vaultRegistry` -> `acceptNewIssues(KSM, KBTC, No)`.


For self-minting, batch the following extrinsics:
1. `vaultRegistry` -> `acceptNewIssues(KSM, KBTC, true)`
2. `issue` -> `requestIssue`
3. `vaultRegistry` -> `acceptNewIssues(KSM, KBTC, false)`
