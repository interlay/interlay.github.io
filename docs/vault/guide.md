...
### How to avoid sniping while minting?
- You can go to polkadot.js.org/apps and deactivate accepting new issue requests from Developer -> Extrinsics -> vaultRegistry -> acceptNewIssues(KSM, KBTC, false)
- After 36 hours (normally 24 hours, but blocktimes), all the pending issue request without a BTC transaction will be cancelled
- No one can mint against your collatreal while issuing is turned false so you can add collateral without a worry.
- For self-minting, you can batch:
